...
`adc-mcp --design <path> [--gated]` — stdio transportのMCPサーバー(別バイナリ)。
ツール: design_read / design_patch / build_and_check / evidence_query /
narrow_param / explain。--gatedはdesign_patchを全Pass時のみ適用
(無人・自動適用の安全装置。対話は非gated=人間のPRレビューがゲート)。
仕様は docs/design-notes/m6-1-mcp-server.md。
