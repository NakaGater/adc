...
**M5回収バックログ**(先行ユニットからの繰越、M5計画時に必ず棚卸しする):

- **Angle mateのE2E受入**(M3繰越): T1語彙では傾斜参照面を構成できず、M3では
  補正数学の単体テストのみでカバー。M5-1の板金フィーチャー(曲げ)で傾斜面が
  語彙に入った時点でE2Eフィクスチャを追加する
- **「座ぐり深さ≥板厚」警告チェッカー**(M1繰越、docs/occt-gotchas.md #4):
  Counterbore貫通退化の警告。MVP外だがM5のチェッカー拡張と同時に検討
