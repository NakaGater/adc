...
**M5回収バックログ**(先行ユニットからの繰越。2026-07-12 M5完了時に棚卸し済み):

- ~~**Angle mateのE2E受入**(M3繰越)~~: **回収済み (M5-1)** —
  45°Flangeのinner面を参照面にしたE2E(crates/adc-check/tests/m5_1_angle_e2e.rs)
- **「座ぐり深さ≥板厚」警告チェッカー**(M1繰越、docs/occt-gotchas.md #4):
  **未回収**。M5-2は板金規則に集中したため、切削系DFM警告はM6の
  ToolAccess/MinCornerRadiusと同時に検討する(繰越を明示)
