...
- ~~§9サンプルの cb_depth(6.5) は板厚より深く座ぐりが実質貫通穴になる~~
  → **サンプル修正済み(2026-07-12)**: ボルト穴はSimple φ6.6貫通
  (六角ボルト+平ワッシャ想定、rationale r_bolt)。Counterbore被覆は
  テスト(m1_2)で維持。**「座ぐり深さ≥板厚」警告チェッカーはMVP外バックログ**
- **Bnd_Boxは既定でgap(数値余裕、1e-7)を含んだ角を返す**(M2-1実測:
  バウンディングボックス寸法が2e-7膨らみ、量子化後も 0.3000002 のような値になる)。
  kernel `bounding_box` はgapを除去して返す
