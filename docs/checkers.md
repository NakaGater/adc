...
- **一方向保証(近似手法の保証範囲)**: **検出した違反は真。未検出は薄肉なしを
  ...
  見逃しうる = false negativeあり)。**検出対象は(反)平行面間の壁のみ**
  (対向面条件5°。5°超で交わる楔状薄肉は検出しない — 2026-07-12 M4承認時に明記)。
  この注記を出力(Evidence note)にも常に含める
- **M2-4時点の制約**: サンプリング対象は平面フェイスのみ。曲面フェイスの
  サンプリングは将来拡張(本書を更新してから実装)
- **責務分担 (M5-1承認時に明記)**: **曲げ部(円筒面)の板厚保証は SheetMetalRules
  の担当**(板金は工程上、曲げ部の板厚=素材厚が構造的に保たれるため代数で厳密に
  検証できる。WallThicknessは平面限定のレイキャスト近似であり曲げ部を見ない)
...
## tolerance_stack_1d (M5-3)

- **判定**: Dim宣言からの**代数計算のみ**(実測しない — 05-schema.md §7.1)。
  worst-case区間 [Σmid − Σhalf, Σmid + Σhalf](RSSは √Σhalf²)⊆ target
- **符号規約・連結性・はめあいテーブル**: 05-schema.md §7.1/§7.2 が正
- **method: Both** の判定はworst-case側(RSS ⊆ WC)。Evidenceに両区間を明記
- **measured / threshold**: Triple [合成lo, Σmid, 合成hi] / Triple [target.0, 中央, target.1]
- **margin** = min(合成lo − target.0, target.1 − 合成hi) / (target半幅)。
  target幅 < 1e-12 は Inconclusive(退化した許容範囲)
- **Inconclusive**: 未知のはめあい記号・サイズ域外(>120mm)・nominal評価失敗・
  target退化
- **Evidence**: anchors=[dim id列]、note=各Dimの寄与(mid±half)とWC/RSS区間。
  経路のfrom/to(端点アンカー)もnoteに明記
- **キャッシュ**: 結果キャッシュキーに経路上のDim正準形+参照param解決値を含める
  (Dim変更・Open割当変更でミスになる)

## sheet_metal_rules (M5-2)

- **性格**: フィーチャー定義からの**代数計算のみ**(高速・厳密。ジオメトリ不要)。
  対象部品が `process: SheetMetal` でなければ Inconclusive
- **規則(MVP固定値。将来は材料テーブル化)**:
  1. **最小曲げ半径**: bend_r ≥ **1.0 × t**(k=1.0)
  2. **フランジ最小長**: length(平坦部) ≥ **4 × t**
  3. **穴-曲げ距離**: 穴/Cutout(Circ)の縁から曲げ根元線まで ≥ **2t + bend_r**。
     代数評価できる配置のみ検査: BaseFlange(Rect)のtop/bottom面上に
     center()/xy()配置されたSimple Hole/Circ Cutout。評価対象外の穴は
     noteに「未評価 n件」と明記(黙って通さない)
- **判定**: 評価した全規則が満たされればPass。曲げなし部品はPass(margin=1、note明記)
- **measured / threshold**: 最悪規則の実測値 / その規則値
- **margin** = min_規則 (実測 − 規則値) / 規則値
- **Evidence(規則違反ごと)**: anchors=[feature_id]、note=実測値+規則値+規則名。
  t / bend_r / length のparam参照(Open含み)はそのまま3点評価に乗る
- **曲げ部の板厚保証は本チェッカーの担当**(wall_thicknessの節を参照)

## 未実装(M6)

ToolAccess/MinCornerRadius — 現状は Inconclusive{"チェッカー未実装"} を返す。
