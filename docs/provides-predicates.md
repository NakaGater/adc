...
- `at` は**明示必須**(2026-07-12決定(a): 暗黙の既定規則は導入しない。§9サンプルも修正済み)

## T2 板金フィーチャー (M5-1、2026-07-12設計メモ承認)

構築方式は**プリミティブ合成**(docs/design-notes/m5-1-sheet-metal.md 案B):
曲げ部=円筒環セグメント(外筒−内筒を角度ウェッジで切出し)、平坦部=プリズム、
fuseで接合。各接合は**面同士の共有**(リング断面=板の側面、リング終端断面=
平坦部始端断面)であり、接線接触ではない。

| feature | 要素 | 述語(結果ソリッド上の同定規則) |
|---|---|---|
| BaseFlange | face: top / bottom / ±x / ±y | Blockと同一(面法線の世界軸分類)。**profile中心=配置原点** |
| Flange | face: bend_inner | 曲げ部の円筒面のうち軸からの距離 ≈ bend_r(内側) |
| Flange | face: bend_outer | 同、距離 ≈ bend_r + t(外側) |
| Flange | face: inner | 平坦部プリズムの曲げ中心側の面(法線 = −radial(angle)) |
| Flange | face: outer | 同、反対側(法線 = +radial(angle)) |
| Flange | face: tip | 平坦部先端の小口(法線 = 接線方向) |
| Cutout | face: wall(Circ)/ walls(Rect、FaceSet) | Pocket踏襲(工具側面のModified) |
| Relief | — | providesなし(参照点にしない) |

- Flangeの曲げ向き: **EdgeSelector第1引数の面の反対側へ曲がる**。
  第1引数面の法線 = 曲げの回転始点、共有エッジの相手面の法線 = フランジの張出方向
- 同定はフューズ**前**の工具ソリッド上で幾何分類し、fuse Historyで前送りする
  (Cylinder/Boss/Counterboreと同型)
- 接線シーム(円筒面と板面の接線)でのfuse分割が実測された場合は
  occt-gotchas.mdに記録し、束縛はAmbiguous原則どおり(2026-07-12指示)
...
- `edges_of(<binding>)` = 束縛面(集合可)の**外周ワイヤのエッジのみ**
  (2026-07-12確定: §9サンプルの意図=外形の丸め。内周ループ=穴リム等を含む解釈では
  サンプルがOCCTで実行不能だった — docs/occt-gotchas.md §5)。
